...
const hexToStyle = (hex: string): React.CSSProperties => ({ color: `#${hex}` });
const hexToBgStyle = (hex: string): React.CSSProperties => ({ backgroundColor: `#${hex}` });
...
const THEMES: Record<DocxTemplateId, {
  ...
  p: string;
  liItem: string;
  table: { container: string; table: string; thead: string; th: string; tbody: string; tr: string; trEven: string; td: string; };
  ...
  corporate: {
    ...
    table: { container: "my-6 rounded-sm overflow-hidden border", table: "w-full text-left text-sm border-collapse", thead: "", th: "px-4 py-3 text-xs font-bold uppercase tracking-wider border-r last:border-r-0 border-b", tbody: "bg-white", tr: "border-b last:border-b-0", trEven: "", td: "px-4 py-3 border-r last:border-r-0 align-top" },
  ...
  modern: {
    ...
    table: { container: "my-8 rounded-lg overflow-hidden shadow-lg border", table: "w-full text-left text-sm", thead: "", th: "px-6 py-3 text-xs font-bold uppercase tracking-wider", tbody: "bg-white", tr: "border-b last:border-0", trEven: "", td: "px-6 py-3 align-top" },
  ...
  executive: {
    ...
    p: "mb-4 leading-relaxed text-justify",
    liItem: "flex items-start mb-2",
    table: { container: "my-6 rounded overflow-hidden border shadow", table: "w-full text-left text-sm border-collapse", thead: "", th: "px-4 py-3 text-xs font-bold uppercase tracking-wider border-r last:border-r-0 border-b", tbody: "bg-white", tr: "border-b last:border-b-0", trEven: "", td: "px-4 py-3 border-r last:border-r-0 align-top" },
...
const BULLET_STYLES: Record<DocxTemplateId, React.CSSProperties> = {
  ...
  executive: { backgroundColor: `#${TEMPLATE_COLORS.executive.listBullet}` },
  briefing: { backgroundColor: `#${TEMPLATE_COLORS.briefing.listBullet}` },
};
...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, template }) => {
  ...
  const parseMarkdown = (text: string) => {
    ...
    const renderFormatting = (value: string) => {
      const parts = value.split(/(\*\*.*?\*\*|\*.*?\*)/g);
      return parts.filter(p => p).map((part, idx) => {
        if (part.startsWith("**") && part.endsWith("**") && part.length >= 4) return <strong key={idx} style={hexToStyle(colors.subtitle)}>{part.slice(2, -2)}</strong>;
    ...
    while (i < lines.length) {
      ...
      if (trimmed === "") { elements.push(<div key={`empty-${i}`} className="h-4" />); i++; continue; }
      ...
      if (hasPipes && isSeparator) {
        ...
        i += 2;
        while (i < lines.length && lines[i].trim().includes("|")) { const cells = parseCells(lines[i]); if (cells.length > 0) rows.push(cells); i++; }
        elements.push(<div key={`table-${i}`} className={theme.table.container} style={{ borderColor: `#${colors.border}` }}><table className={theme.table.table}><thead className={theme.table.thead} style={hexToBgStyle(colors.headerBg)}><tr style={{ borderColor: `#${colors.border}` }}>{rows[0]?.map((cell, idx) => <th key={idx} className={theme.table.th} style={{ borderColor: `#${colors.border}`, ...hexToStyle(colors.headerText) }}>{cell}</th>)}</tr></thead><tbody className={theme.table.tbody}>{rows.slice(1).map((row, rowIdx) => <tr key={rowIdx} className={`${theme.table.tr} ${rowIdx % 2 === 1 ? theme.table.trEven : ""}`} style={{ backgroundColor: rowIdx % 2 === 1 ? `#${colors.rowEvenBg}` : `#${colors.pageBg}`, borderColor: `#${colors.border}` }}>{row.map((cell, cellIdx) => <td key={cellIdx} className={theme.table.td} style={{ borderColor: `#${colors.border}`, ...hexToStyle(colors.rowText) }}>{renderFormatting(cell)}</td>)}</tr>)}</tbody></table></div>);
        continue;
      }
  ...
  return <div className={theme.page} style={{ backgroundColor: `#${colors.pageBg}` }}>{parseMarkdown(content)}</div>;
};
